...
def list_branches(project_id: str, plan_id: str | None = None, container=Depends(get_container)):
...
def get_branch(project_id: str, branch_id: str, container=Depends(get_container)):
    with container.uow_factory.read_only(project_id) as uow:
...
def create_branch(project_id: str, body: BranchCreateRequest, container=Depends(get_container)):
    from cardre.application.governance.create_branch import CreateBranchCommand

    result = container.create_branch_factory(project_id)(CreateBranchCommand(
...
def list_comparisons(project_id: str, plan_id: str | None = None, container=Depends(get_container)):
    with container.uow_factory.read_only(project_id) as uow:
...
def get_comparison(project_id: str, comparison_id: str, container=Depends(get_container)):
    with container.uow_factory.read_only(project_id) as uow:
...
def create_comparison(project_id: str, body: ComparisonCreateRequest, container=Depends(get_container)):
    from cardre.application.governance.create_comparison import CreateComparisonCommand

    result = container.create_comparison_factory(project_id)(CreateComparisonCommand(
...
def refresh_comparison(project_id: str, comparison_id: str, container=Depends(get_container)):
    from cardre.application.governance.refresh_comparison import RefreshComparisonCommand

    container.refresh_comparison_factory(project_id)(RefreshComparisonCommand(project_id=project_id, comparison_id=comparison_id))
    with container.uow_factory.read_only(project_id) as uow:
        comparison = uow.comparisons.get_comparison(comparison_id)
    if comparison is None:
        raise CardreApiError(code=ErrorCode.COMPARISON_NOT_FOUND, message=f"Comparison {comparison_id!r} not found.", status_code=404)
...
def get_champion(project_id: str, plan_id: str | None = None, container=Depends(get_container)):
    with container.uow_factory.read_only(project_id) as uow:
        if plan_id:
...
def assign_champion(project_id: str, body: ChampionAssignmentRequest, container=Depends(get_container)):
    from cardre.application.governance.assign_champion import AssignChampionCommand

    container.assign_champion_factory(project_id)(AssignChampionCommand(
