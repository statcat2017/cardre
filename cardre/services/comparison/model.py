...
def build_model_comparison(
    ...
    lr_b = find_typed_artifact(ctx.store, ctx.step_map_baseline, "model-fit", ctx.plan_version_id_baseline, None, (EvidenceKind.MODEL_ARTIFACT, EvidenceKind.ENSEMBLE_MODEL_ARTIFACT))
    lr_c = find_typed_artifact(ctx.store, ctx.step_map_challenger, "model-fit", ctx.plan_version_id_challenger, ctx.branch_id_challenger, (EvidenceKind.MODEL_ARTIFACT, EvidenceKind.ENSEMBLE_MODEL_ARTIFACT))
    ...
    if b_family == "logistic_regression" and c_family == "logistic_regression":
        ...
        for var_name in sorted(set(b_coeffs) | set(c_coeffs)):
            ...
            model_vars.append({
                ...
                "challengers": {ctx.branch_id_challenger: {"included": var_name in c_coeffs, "coefficient": c_val, "points_range": 0}},
            })
