...
- **EvidenceAdapter**: the concrete thing at the evidence seam — one per `EvidenceKind`, owns matching (which artifact in a list is this kind) and parsing (artifact bytes → typed dataclass). Registered in `cardre/adapters/evidence/parsers.py` via the `EVIDENCE_ADAPTERS` registry. `cardre/adapters/evidence/reader.py` is the thin dispatcher + `artifact_lineage` resolver that delegates to adapters.
