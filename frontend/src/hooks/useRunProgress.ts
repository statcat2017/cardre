...
export function useRunProgress(projectId: string, onRunComplete: () => void): UseRunProgressReturn {
  ...
  const startRun = useCallback(
    async (planVersionId: string, options?: RunOptions) => {
      setRunning(true);
      setError(null);
      setRunStalled(false);
      setLiveDiagnostic(null);
        ...
        const pollOnce = async () => {
          ...
          try {
            ...
            if (pollErr?.requestId) {
              addDiagnostic(
                `Poll error (${pollErr.code}, req=${pollErr.requestId.slice(0, 8)}): ${pollErr.message}`,
              );
            } else {
              addDiagnostic(
                `Poll error (${isApiError(e) ? (e as ApiError).code : "UNKNOWN"}): ${isApiError(e) ? (e as ApiError).message : String(e)}`,
              );
            }
  ...
  const stepProgress =
    running && totalPlanSteps > 0 ? { completed: progressCompleted, total: totalPlanSteps } : null;
