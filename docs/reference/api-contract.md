...
The service layer (`cardre/services/`) uses plain dataclass DTOs defined in `cardre/services/plan_dto.py`. These mirror the Pydantic models in `sidecar/models.py` but keep the service layer free of FastAPI dependencies. The route layer converts between them via `dataclasses.asdict()`.
