...
The reporting system produces report bundles from immutable run artifacts. It is a read-only consumer of run evidence — it must not become a second modelling execution path.
...
| `contracts.py` | `cardre/application/reporting/contracts.py` | Canonical step IDs and required-steps constants |
...
1. **Readiness check**: verifies all required evidence is available for the run.
...
| `source` | `ReportSource` | Run manifest references |
...
| `cutoffs` | `CutoffInfo` | Cutoff analysis results |
...
Readiness checks verify that all required evidence is available for the run.
