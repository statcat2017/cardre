...
REJECT_INFERENCE_PATHWAY = PathwaySpec(
    name="Reject Inference Pathway",
    description="Full credit scorecard pathway with reject inference support (TTD only)",
    phases=[
        # Phase 2A + RI: Import through Variable Clustering/Selection
        # validate-target is placed AFTER reject-inference-none so null-target
        # rows (rejects) are filtered out before validation.
        [
            PathwayStepSpec("import", "cardre.import_dataset"),
            PathwayStepSpec("define-metadata", "cardre.define_modelling_metadata",
                            params={
            ...
            PathwayStepSpec("sample-definition", "cardre.development_sample_definition",
                            ...
                            parent_step_ids=["apply-exclusions", "define-metadata"]),
            PathwayStepSpec("define-reject-population", "cardre.define_reject_population",
                            params={"exclusion_categories": {}},
                            parent_step_ids=["sample-definition", "define-metadata", "apply-exclusions"]),
            PathwayStepSpec("reject-inference-none", "cardre.reject_inference_none",
                            params={},
                            parent_step_ids=["define-reject-population"]),
            PathwayStepSpec("validate-target", "cardre.validate_binary_target",
                            params={"target_column": ""},
                            parent_step_ids=["reject-inference-none", "define-metadata"]),
            PathwayStepSpec("split", "cardre.split_train_test_oot", node_version="2",
                            params={
                                "strategy": "random_stratified",
                                "train_fraction": 0.6, "test_fraction": 0.2,
                            ...
                            parent_step_ids=["apply-exclusions", "validate-target", "define-metadata"]),
            PathwayStepSpec("explicit-missing-outlier-treatment",
                            "cardre.explicit_missing_outlier_treatment", category="apply",
                            params={"imputations": {}, "caps": {}, "floors": {}},
                            parent_step_ids=["split"]),
        ...
        [
            PathwayStepSpec("technical-manifest-stub", "cardre.technical_manifest_export",
                            parent_step_ids=[
                                "define-metadata", "sample-definition",
                                "define-reject-population", "reject-inference-none",
...
def register_reject_inference_pathway(store: ProjectStore, project_id: str) -> str:
    """Register the reject-inference-enabled scorecard pathway."""
    plan_id = store.create_plan(project_id, REJECT_INFERENCE_PATHWAY.name)
    steps = build_pathway_steps(REJECT_INFERENCE_PATHWAY)
    store.create_plan_version(plan_id, steps, description=f"Auto-registered {REJECT_INFERENCE_PATHWAY.name}")
    return plan_id
