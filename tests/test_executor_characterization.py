...
The reuse/carry-forward path has no public caller today.  Reuse tests
explicitly pin current *internal* behaviour at the private action-execution
seam, not a public contract.
"""
...
class TestToNodeScoping:

    def test_run_to_node_executes_only_ancestor_closure(self, tmp_path):
        """run_to_node creates run_steps only for the ancestor closure
        of the target step, not for the full plan."""
        store = _make_store(tmp_path)
        input_path = _write_input_csv(tmp_path)
        project_id, plan_id = _seed_project_and_plan(store)

        pv_id = _seed_plan_version_with_custom_steps(store, plan_id, [
            _import_step("step-a", source_path=input_path),
            _profile_step("step-b", parent="step-a", position=1),
            _noop_step("step-c", "step-b", 2),
        ])

        run_id = RunRepository(store).create(pv_id)
        executor = PlanExecutor(store)
        executor.run_to_node(pv_id, "step-b", run_id, force=True)

        result_steps = RunStepRepository(store).get_for_run(run_id)
        result_ids = [rs.step_id for rs in result_steps]
        assert "step-a" in result_ids
        assert "step-b" in result_ids
        assert "step-c" not in result_ids

        # Evidence edges should exist for a -> b
        edges = store.execute(
            "SELECT DISTINCT parent_step_id FROM evidence_edges WHERE run_id = ?",
            (run_id,),
        ).fetchall()
        parent_set = {r["parent_step_id"] for r in edges}
        assert "step-a" in parent_set

    def test_run_to_node_root_target(self, tmp_path):
        """Running to-node on the root step executes only that step."""
        store = _make_store(tmp_path)
        input_path = _write_input_csv(tmp_path)
        project_id, plan_id = _seed_project_and_plan(store)
        ...
        run_id = RunRepository(store).create(pv_id)
        executor = PlanExecutor(store)
        executor.run_to_node(pv_id, "step-a", run_id, force=True)

        result_steps = RunStepRepository(store).get_for_run(run_id)
        result_ids = [rs.step_id for rs in result_steps]
        assert result_ids == ["step-a"]


# =========================================================================
# Reuse / carry-forward path (private seam — no public caller)
# =========================================================================


class TestReusePath:

    def test_reuse_copies_run_step_with_carry_forward_markers(self, tmp_path):
        """A reuse action copies the prior run_step and annotates its
        fingerprint with cardre_step_carried_forward markers."""
        store = _make_store(tmp_path)
        input_path = _write_input_csv(tmp_path)
        project_id, plan_id = _seed_project_and_plan(store)
        ...
        run_1 = RunRepository(store).create(pv_id)
        executor = PlanExecutor(store)
        executor.run_plan_version(pv_id, run_1)

        # Second run: reuse actions
        steps = PlanRepository(store).get_version_steps(pv_id)
        run_2 = RunRepository(store).create(pv_id)

        reuse_actions = [_StepAction(spec=s, action="reuse",
                                      evidence_source=None)
                         for s in steps]

        has_failure, outputs, records = executor._execute_actions(
            pv_id, run_2, reuse_actions,
        )
        assert has_failure is False

        for step_spec in steps:
            rs = records.get(step_spec.step_id)
            assert rs is not None, f"No reused step for {step_spec.step_id}"
            fp = rs.execution_fingerprint
            assert fp.get("cardre_step_carried_forward") is True
            assert fp.get("carried_forward_from_run_step_id") is not None
            assert fp.get("carried_forward_from_plan_version_id") is not None
            assert fp.get("carried_forward_from_run_id") is not None

    def test_reuse_copies_evidence_as_reused(self, tmp_path):
        """Reuse copies evidence edges with is_reused=1 and preserves
        ...
        original_edges = store.execute(
            "SELECT * FROM evidence_edges WHERE run_id = ?", (run_1,)
        ).fetchall()
        assert len(original_edges) >= 1
        original_edge = original_edges[0]
        original_artifacts = store.execute(
            "SELECT * FROM evidence_artifacts WHERE evidence_edge_id = ?",
            (original_edge["evidence_edge_id"],),
        ).fetchall()

        # Reuse in a second run
        steps = PlanRepository(store).get_version_steps(pv_id)
        run_2 = RunRepository(store).create(pv_id)
        reuse_actions = [_StepAction(spec=s, action="reuse",
                                      evidence_source=None)
                         for s in steps]
        executor._execute_actions(pv_id, run_2, reuse_actions)

        copied_edges = store.execute(
            "SELECT * FROM evidence_edges WHERE run_id = ?", (run_2,)
        ).fetchall()
        assert len(copied_edges) == len(original_edges)
        for e in copied_edges:
            assert e["is_reused"] == 1
            assert e["is_stale"] == original_edge["is_stale"]
            assert e["stale_reason"] == original_edge["stale_reason"]

        for copied_edge in copied_edges:
            copied_arts = store.execute(
                "SELECT * FROM evidence_artifacts WHERE evidence_edge_id = ?",
                (copied_edge["evidence_edge_id"],),
            ).fetchall()
            for ca in copied_arts:
                matching = [
                    oa for oa in original_artifacts
                    if oa["artifact_id"] == ca["artifact_id"]
                ]
                assert matching, (
                    f"Copied artifact {ca['artifact_id']} not found in original"
                )

    def test_reuse_copies_lineage(self, tmp_path):
        """Reuse copies artifact_lineage rows into the new run."""
        store = _make_store(tmp_path)
        input_path = _write_input_csv(tmp_path)
        project_id, plan_id = _seed_project_and_plan(store)
        ...
        copied_count = store.execute(
            "SELECT COUNT(*) FROM artifact_lineage WHERE run_id = ?",
            (run_2,),
        ).fetchone()[0]
        assert copied_count >= 1

        sample = store.execute(
            "SELECT step_id, direction, run_id FROM artifact_lineage "
            "WHERE run_id = ? LIMIT 1",
            (run_2,),
        ).fetchone()
        assert sample["direction"] in ("input", "output")

    def test_reuse_fallback_to_execute_when_no_prior(self, tmp_path):
        """When no prior evidence exists for a step, a reuse action
        falls back to executing the step normally instead of crashing."""
        store = _make_store(tmp_path)
        input_path = _write_input_csv(tmp_path)
        project_id, plan_id = _seed_project_and_plan(store)
        ...
        reuse_actions = [_StepAction(spec=s, action="reuse",
                         ...
                         for s in steps]
        has_failure, _outputs, records = executor._execute_actions(
            pv_id, run_id, reuse_actions,
        )

        # The step should have been executed via fallback (not reused)
        assert len(records) == 1
        rs = records[steps[0].step_id]
        assert rs is not None
        # A diagnostic was appended showing the fallback
        diags = store.execute(
