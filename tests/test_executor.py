...
class Wave2Tests:
    ...
    def test_branch_execution_reuses_shared_evidence(self) -> None:
        store, tmp = make_store()
        project_id = store.create_project("test")
        plan_id = store.create_plan(project_id, "test-plan")

        shared_step = StepSpec(
            step_id="shared_source", node_type="cardre.test.simple_source",
            node_version="1", category="transform",
            params={}, params_hash=json_logical_hash({}),
            parent_step_ids=[], branch_label="", position=0,
        )
        branch_step = StepSpec(
            step_id="branch_transform", node_type="cardre.test.simple_transform",
            node_version="1", category="transform",
            params={}, params_hash=json_logical_hash({}),
        ...
        # First: run full plan to establish shared upstream evidence
        full_run_id = executor.run_plan_version(store, pv_id)
        full_run = store.get_run(full_run_id)
        assert full_run["status"] == "succeeded"
        ...
        # Third: run the branch (force to bypass staleness check since
        # the full run already executed all steps)
        ...
        # Fourth: branch run should re-execute branch-owned step but not
        # shared upstream (reuses the full-run evidence)
        branch_steps = store.get_run_steps(branch_run_id)
        branch_step_ids = {rs.step_id for rs in branch_steps}
        assert "branch_transform" in branch_step_ids
        for rs in branch_steps:
            if rs.step_id == "branch_transform":
                assert rs.status == "succeeded"
                break
