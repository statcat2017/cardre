...
class TestRunStepWriter:
    ...
    def test_reused_step_copies_evidence_and_lineage(self, tmp_path):
        """Reused step copies evidence edges, artifacts, and lineage rows with correct provenance."""
        store = _make_store(tmp_path)
        ...
        store.execute(
            "INSERT INTO projects (project_id, name, created_at, cardre_version) VALUES (?, ?, ?, ?)",
            (project_id, "Reuse Full Test", now, "0.2.0"),
        )
        plan_id = str(uuid.uuid4())
        store.execute(
            "INSERT INTO plans (plan_id, project_id, name, created_at) VALUES (?, ?, ?, ?)",
            (plan_id, project_id, "Reuse Full Plan", now),
        )
        pv_id = str(uuid.uuid4())
        store.execute(
            "INSERT INTO plan_versions (plan_version_id, plan_id, version_number, is_committed, created_at, description) "
            "VALUES (?, ?, 1, 1, ?, ?)",
            (pv_id, plan_id, now, "Reuse full version"),
        )

        step_id = "step-reused-full"
        store.execute(
            "INSERT INTO plan_steps (step_id, plan_version_id, node_type, node_version, category, "
            " params_json, params_hash, branch_label, position, canonical_step_id) "
            "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
        ...
        # Insert a prior artifact (needed for FK in evidence_artifacts)
        store.execute(
            "INSERT INTO artifacts (artifact_id, artifact_type, role, path, physical_hash, logical_hash, media_type, created_at) "
            "VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
            ("art-reuse-1", "dataset", "input", "/tmp/art.csv", "abc", "def", "text/csv", now),
        )

        # Insert a prior evidence edge
        prior_ee_id = str(uuid.uuid4())
        store.execute(
            ...
            "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 0, 0, NULL, ?)",
            (prior_ee_id, prior_run_id, prior_rs_id, pv_id, step_id, "parent-step",
             prior_run_id, prior_rs_id, "exact", "parent_0", now),
        )

        # Insert a prior evidence artifact
        prior_ea_id = str(uuid.uuid4())
        store.execute(
            "INSERT INTO evidence_artifacts (evidence_artifact_id, evidence_edge_id, artifact_id, role, created_at) "
            "VALUES (?, ?, ?, ?, ?)",
            (prior_ea_id, prior_ee_id, "art-reuse-1", "input", now),
        )

        # Insert a prior lineage row
        store.execute(
            "INSERT INTO artifact_lineage (lineage_id, run_id, run_step_id, plan_version_id, step_id, branch_id, "
            " artifact_id, direction, created_at) "
            "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)",
            (str(uuid.uuid4()), prior_run_id, prior_rs_id, pv_id, step_id, None, "art-reuse-1", "output", now),
        )

        # Build the data for write_reused_run_step
        new_run_id = run_repo.create(pv_id)
        new_rs_id = str(uuid.uuid4())
        ...
        # Load the prior edges, artifacts, and lineage
        evidence_repo = EvidenceRepository(store)
        edges = evidence_repo.get_edges_for_run_step(prior_rs_id)
        all_artifacts = evidence_repo.get_artifacts_for_run_step(prior_rs_id)
        from cardre.store.artifact_repo import ArtifactRepository
        lineage_rows = ArtifactRepository(store).get_lineage_for_run_step(prior_rs_id)

        assert len(edges) == 1, "Expected 1 prior evidence edge"
        assert len(all_artifacts) == 1, "Expected 1 prior evidence artifact"
        assert len(lineage_rows) == 1, "Expected 1 prior lineage row"
        ...
        # Verify the new run_step row
        row = store.execute(
            "SELECT run_step_id, run_id, step_id, status FROM run_steps WHERE run_step_id = ?",
            (new_rs_id,),
        ).fetchone()
        ...
        # Verify the new evidence edges: is_reused=1, correct run_id/run_step_id, preserved source provenance
        new_edges = store.execute(
            "SELECT evidence_edge_id, run_id, run_step_id, source_run_id, source_run_step_id, "
            " is_reused, is_stale, stale_reason, policy, source_label, parent_step_id "
            "FROM evidence_edges WHERE run_id = ? AND run_step_id = ?",
            (new_run_id, new_rs_id),
        ).fetchall()
        assert len(new_edges) == 1, f"Expected 1 reused edge, got {len(new_edges)}"
        ne = new_edges[0]
        assert ne["run_id"] == new_run_id
        assert ne["run_step_id"] == new_rs_id
        assert ne["is_reused"] == 1
        assert ne["source_run_id"] == prior_run_id
        assert ne["source_run_step_id"] == prior_rs_id
        assert ne["policy"] == "exact"
        assert ne["source_label"] == "parent_0"
        assert ne["parent_step_id"] == "parent-step"
        assert ne["is_stale"] == 0
        assert ne["stale_reason"] is None

        # Verify the new evidence artifacts: correct edge, preserved role
        new_artifacts = store.execute(
            "SELECT evidence_artifact_id, evidence_edge_id, artifact_id, role "
            "FROM evidence_artifacts WHERE evidence_edge_id = ?",
            (ne["evidence_edge_id"],),
        ).fetchall()
        assert len(new_artifacts) == 1, f"Expected 1 reused artifact, got {len(new_artifacts)}"
        na = new_artifacts[0]
        assert na["artifact_id"] == "art-reuse-1"
        assert na["role"] == "input"

        # Verify the new lineage rows: correct run_id/run_step_id, preserved direction
        new_lineage = store.execute(
            "SELECT artifact_id, direction, run_id, run_step_id, step_id, branch_id "
            "FROM artifact_lineage WHERE run_id = ? AND run_step_id = ?",
            (new_run_id, new_rs_id),
        ).fetchall()
        assert len(new_lineage) == 1, f"Expected 1 lineage row, got {len(new_lineage)}"
        nl = new_lineage[0]
        assert nl["artifact_id"] == "art-reuse-1"
        assert nl["direction"] == "output"
        assert nl["run_id"] == new_run_id
        assert nl["run_step_id"] == new_rs_id
        assert nl["step_id"] == step_id
        assert nl["branch_id"] is None
