...
A single-step parameter editor for draft plan versions. Committed versions
are immutable; unknown steps are rejected.
"""
...
class TestUpdateStepParams:
    def test_updates_draft_step_params(self, provisioned_project):
        ...
        uc = UpdateStepParams(_factory(uow_factory, project_id))
        ...
        assert edited.params == new_params
        assert edited.params_hash == json_logical_hash(new_params)
