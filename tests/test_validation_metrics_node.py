...
pytestmark = pytest.mark.xfail(reason="Old StepRunner path; needs NodeContext update")
...
def test_validation_metrics_step_fails_when_test_sample_required_but_missing(store):
    ...
    result = _run_validation_step(
        store,
        parent_artifacts=[train_dataset, modelling_metadata],
        params={
            "require_test": True,
            "require_oot": False,
    ...
    assert "TEST_SAMPLE_PRESENT" in result.errors[0]["message"]


def test_validation_metrics_step_fails_when_score_column_missing(store):
    modelling_metadata = _write_modelling_metadata(store)
    train_dataset = _write_dataset(
        store,
        role="train",
